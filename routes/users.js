...
router.post("/generateToken", async (req, res, next) => {
  ...
  try {
    ...
    let user = await Users.findOne({
      where: { [Op.and]: [{ name: name }, { password: passwordHash }] },
    });
    let relations = await UsersRoles.findOne({
      where: { [Op.and]: [{ users_iduser: user.iduser }] },
    });
    let roles = await Roles.findOne({
      where: { [Op.and]: [{ idrole: relations.roles_idrole }] },
    });

    /* Genera el token con los datos encriptados */
    const accessToken = jwt.sign(
      { name: user.name, role: roles.name },
